...
test('catalog and auth',async()=>{assert.equal((await app.inject({method:'GET',url:'/api/catalog'})).json().products.length,4);assert.equal((await app.inject({method:'GET',url:'/api/admin/catalog'})).statusCode,401)});
